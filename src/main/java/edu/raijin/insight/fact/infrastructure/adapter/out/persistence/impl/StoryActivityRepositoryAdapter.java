...
public class StoryActivityRepositoryAdapter implements RegisterStoryActivityPort, UpdateStoryActivityPort {
    ...
    @Override
    public Long register(Long eventId, StoryActivity movement) {
        StoryActivityEntity entity = mapper.toEntity(eventId, movement);
        return eventRepository.save(entity).getId();
    }

    @Override
    public Optional<StoryActivity> findByStoryId(UUID storyId) {
        return eventRepository.findByStoryId(storyId).map(mapper::toDomain);
    }

    @Override
    public StoryActivity update(Long dateId, StoryActivity storyActivity) {
        StoryActivityEntity entity = mapper.toEntity(dateId, storyActivity);
